...
def parse_args():
    parser = argparse.ArgumentParser(description="USB 双目相机预览和左右图像采集")
    ...
    parser.add_argument("--width", type=int, default=2560,help="可选：指定相机输出宽度")
    parser.add_argument("--height", type=int, default=720,help="可选：指定相机输出高度")
...
def open_camera(index, backend, width, height, fps):
    ...
    parameters = [
        int(cv2.CAP_PROP_FOURCC), int(fourcc),
        int(cv2.CAP_PROP_FPS), round(fps),
    ]
...
def list_cameras(backend, max_index=10):
    ...
    for index in range(max_index):
        ...
        capture.release()
    if not found:
        print("未发现可用相机。请检查 USB 连接或尝试 --backend msmf")
...
def make_preview(left_frame, right_frame):
    ...
    if left_frame.shape[0] < target_height:
        ...
        left_frame = cv2.copyMakeBorder(
            left_frame, 0, bottom, 0, 0, cv2.BORDER_CONSTANT
        )
    if right_frame.shape[0] < target_height:
        bottom = target_height - right_frame.shape[0]
        right_frame = cv2.copyMakeBorder(
            right_frame, 0, bottom, 0, 0, cv2.BORDER_CONSTANT
        )

    preview = np.hstack((left_frame, right_frame))
    middle = left_frame.shape[1]
    cv2.putText(preview, "LEFT", (20, 40), cv2.FONT_HERSHEY_SIMPLEX,
                1, (0, 255, 0), 2, cv2.LINE_AA)
    cv2.putText(preview, "RIGHT", (middle + 20, 40), cv2.FONT_HERSHEY_SIMPLEX,
                1, (0, 255, 0), 2, cv2.LINE_AA)
    return preview
...
def save_pair(output_dir, left_frame, right_frame):
    ...
    if not cv2.imwrite(str(left_path), left_frame):
        raise RuntimeError(f"保存失败: {left_path}")
    if not cv2.imwrite(str(right_path), right_frame):
        raise RuntimeError(f"保存失败: {right_path}")
    ...
    def update_frame(self):
        ...
        if self.status_label.text() == "等待相机画面……":
            self.status_label.setText(
                f"双目画面：{width} × {height}，点击“拍照”保存标定图片"
            )
    ...
    def capture_photo(self):
        ...
        try:
            left_path, right_path = save_pair(
                self.output_dir, self.left_frame, self.right_frame
            )
        except RuntimeError as error:
            QMessageBox.critical(self, "保存失败", str(error))
            return
        self.status_label.setText(
            f"拍照成功：{left_path.name} / {right_path.name}"
        )
...
def run(args):
    ...
    try:
        if args.mode == "sbs":
            capture = open_camera(args.camera, args.backend, args.width,
                                  args.height, args.fps)
            captures.append(capture)
            read_frames = lambda: read_side_by_side(capture)
        else:
            left_capture = open_camera(args.left, args.backend, args.width,
                                       args.height, args.fps)
            captures.append(left_capture)
            right_capture = open_camera(args.right, args.backend, args.width,
                                        args.height, args.fps)
            captures.append(right_capture)
